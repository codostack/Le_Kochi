...
const DosaRestaurantCard = () => {
  ...
  return (
<div className="min-h-screen bg-[#06120a] text-[#eab308] pt-4 sm:p-6 md:p-8">
        {/* Section Header */}
...
<div className="h-px bg-[#1a2b1f] flex-grow" />
      </div>
...
<div
  key={item.id}
  className="bg-[#0b1a10] border border-[#1a2b1f] overflow-hidden shadow-xl hover:border-[#eab308] transition-all duration-300"
>
      {/* Image Container */}
<div className="relative aspect-[16/10] w-full bg-[#102016]">
        <img
          src={item.image}
          alt={item.title}
        ...
        <div className="absolute -bottom-5 right-4 z-10">
          <div
            className={`${item.logoBg} w-16 h-16 rounded-full flex flex-col items-center justify-center p-2 text-center shadow-lg`}
          >
...
<h3 className="text-xl font-bold tracking-tight text-[#eab308] mb-1">
  {item.title}
</h3>
        ...
        {/* Footer */}
<div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-[#eab308] font-semibold">
          <svg
